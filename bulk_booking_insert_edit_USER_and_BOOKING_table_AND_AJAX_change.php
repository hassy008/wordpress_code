...
class OpsEarlyBirdBooking extends AbstractModule {
	...
	//get total bulk amount 
	public function bulkTotal($user_id)
	{
		//here we need to show the amount from payments_table
		$early_bird_option_3 = EARLY_BIRD_DEPOSIT_OPTION_3;
...
class OpsAction
{
        ...
        public function updateBulk()
    {
        ...
        $member_id = $_POST['member_id']; //we need member ID bcaz here we are updating each of member ID and we got member ID using AJAX call
	...
	//check user by email
        ...
        if (!$user) {
         ...
         $OpsBulkBooking     = new OpsBulkBooking();
            ...
            // $return             = $OpsBulkBooking->insert($data);
            $return             = $OpsBulkBooking->updateOrInsert($data, ['member_id' => $user_id]);
...
class OpsAjaxAction
{
    ...
    public function updateBulk()
    {
        ...
        $user = get_user_by( 'ID', $response->user_id ); //$user get all the usermeta data
