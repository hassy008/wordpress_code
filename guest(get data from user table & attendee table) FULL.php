...
class OpsDashAjaxAction
{
	...
	{
		$self = new self();
		add_action("wp_ajax_get_guest", array($self, 'getGuest'));
	}
	...
	public function getGuest()
	{
        ...
        OpsAuthorization::dashboard(get_current_user_id()); //only authorized person can able to do any action


        $guest_id = intval($_POST['guest_id']);

        $OpsAttendee = new OpsAttendee();
		$response = $OpsAttendee->getRow(['id' => $guest_id]);

		wp_send_json_success(['guest' => $response]);
...
class OpsAuthorization {
	public static function dashboard( $user_id ) {
		...
		$permitted_role = [
			//array name from inside wp-admin -> role->inspect->get value
			'administrator', 'editor', 'author', 'webmaster'
		];

		$result = array_intersect($permitted_role, (array)$user->roles);

		if(empty($result)){
			die("You are permitted to this action");
		}
	}
}
